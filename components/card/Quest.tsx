...
export const Quest = (quest: QuestRes) => {
  const handelColor = (s: SelectorOption[], value: string) => {
    console.log(s.find((item) => item.value === value)?.color);

    return s.find((item) => item.value === value)?.color;
  };
  ...
  return (
    <>
      <article className="bg-white rounded-xs p-5 tablet-l:w-56 tablet-l:h-52 flex flex-col group relative">
        ...
        {/* <div className="relative">
          <div
            style={{ backgroundColor: handelColor(CATEGORIES, quest.category) }}
            className="w-30 h-8.75 absolute -left-5 bottom-0 rounded-tr-m rounded-br-m tablet-l:w-22 tablet-l:h-6.25 "
          />
          <span className="absolute bottom-0  left-0 tablet-l:text-caption text-xs">
            {quest.category}
          </span>
          <ActionBtn open={open} />
        </div> */}
